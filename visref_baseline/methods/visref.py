...
def run_visref(sample: dict[str, Any], adapter, cfg: dict[str, Any]) -> dict[str, Any]:
    start = time.perf_counter()
    prompt_cfg = cfg["prompt"]
    vis_cfg = cfg["visref"]

    state = adapter.start_reasoning(sample["question"], sample["image"], prompt_cfg)
    visual_tokens = adapter.encode_image(sample["image"])
    ...
    for step in range(1, vis_cfg["max_steps"] + 1):
        _, state = adapter.generate_reasoning_step(state, extra_visual_tokens=extra_visual_tokens)

        z_k = adapter.get_reasoning_text_embeddings(state)
        M_k = build_Mk(z_k)
